...
const seedDatabase = () => {
  ...
  for (let i = 0; i < 3000; i++) {
    ...
    const queryString = `INSERT INTO reviews (rating, comment, date, username, usercity, avartar, restaurant_id) VALUES ("${rating}", "${comment}", STR_TO_DATE("${newDate}", "%m/%d/%Y"), "${username}", "${userCity}", "${avatar}", ${reataurantId});`
    db.query(queryString);
  }
